...
std::complex<float> cmul (std::complex<float> x, float y) {
	...
	airspyHandler::airspyHandler (int32_t	inputRate,
	                              int32_t	frequency,
...
//      (selectedRate / 1000) vs (2400000 / 1000)
...
int32_t  i, j;
	...
	for (i = 0; i < nSamples; i ++) {
	   convBuffer [convIndex ++] = std::complex<float> (sbuf [2 * i] / (float)2400,
	                                           sbuf [2 * i + 1] / (float)2400);
	   if (convIndex >= convBufferSize) {
	      ...
	      theBuffer -> putDataIntoBuffer (temp, 2400);
